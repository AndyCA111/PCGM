...
def get_metadata(path="/home/wpeng/data/MRI_High/"):
    seg_path = '/home/wepeng/binx/synthseg/'
    ...
    for index, row in df_sri.iterrows():
        ...
        if subject_visit_key in df_seg_sri_subjects:
            ...
            seg_fn = 'Lab_seg/' + subject_visit_key + '_synthseg.nii.gz'
            path_here = path + fn
            seg_path_here = seg_path + seg_fn
...
video_data_paths_dict = {
    ...
    "synthseg" : "/home/wepeng/binx/synthseg/",
    "MRI": "/home/wepeng/data/data/MRI_3Set/",#/home/Nobias/data/hand_orig/ADNI_extract/img_orig_longitudinal/
}
...
def create_dataset(dataset_name, img_size=176, T=None, prompt='a mri brain image', deterministic=False, num_workers=1, return_dataset=False, ifexample = False,resize = False, age_normalize=False):
    data_path = video_data_paths_dict[dataset_name]
    T = default_T_dict[dataset_name] if T is None else T
    if dataset_name == "MRI":
        dataset = MRIDataset(train=True, imgsize=img_size, path=data_path, shard=0, num_shards=0, T=T, prompt=prompt, ifexample = ifexample,resize=resize, age_normalize= age_normalize)
    else:
...
class MRIDataset(BaseDataset):
    def __init__(self, path, train=None, shard=None, num_shards=None, T=None, resize=False, imgsize =176, prompt ='a brain image',ifexample=False,age_normalize=True):
...
def get_data_loaders(
    ...
    dataset = create_dataset(dataset_name, img_size=img_size, prompt=prompt, ifexample = ifexample,resize=resize,age_normalize=age_normalize)
