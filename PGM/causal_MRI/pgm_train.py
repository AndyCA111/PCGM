...
from pgm.pgm_utils import seed_all, seed_worker, EMA
import os
import copy
import argparse
import pyro
...
from pgm.pgm_utils import update_stats, calculate_loss
from pgm.pgm_layer import TraceStorage_ELBO
from torch.optim.lr_scheduler import ReduceLROnPlateau
from pgm.pgm_dataloader import BrainDataset
...
def preprocess(batch, device="cuda:0"):
    ...
    for k, v in batch.items():
        if k == 'x':
            batch_new['x'] = batch['x'].float().to(device)  # [0,1]
        elif k in ['age']:
...
def sup_epoch(model, ema, dataloader, elbo_fn, optimizer=None, setup=None, is_train=True, loss_norm="l1"):
    ...
    loader = tqdm(enumerate(dataloader), total=len(
        dataloader), miniters=len(dataloader)//10, mininterval=5)
...
def eval_epoch(model, dataloader):
    ...
    for i, k in enumerate(model.variables.keys()):
        ...
        if k in ['sex','finding']:
            ...
            assert targets_k.size()==preds_k.size(), f"{k} size doesn't match, targets {targets_k.size()} preds {preds_k.size()}"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_path',
                        help='Path to load checkpoint.', type=str, default="None")##../../checkpoints/checkpoint.pt
    parser.add_argument('--setup',  # sup_pgm/sup_aux/sup_determ
                        help='training setup.', type=str, default='sup_pgm')
    ...
    parser.add_argument('--epochs',
                        help='Number of training epochs.', type=int, default=500000)
    ...
    parser.add_argument('--input_res',
                        help='Input image crop resolution.', type=int, default=192)
    ...
    if not args.testing:
        ...
        for epoch in range(args.epochs):
            ...
            if args.setup == 'sup_pgm' or args.setup == 'sup_aux' or args.setup=="sup_determ":
                stats = sup_epoch(
                    model, ema,
                    dataloaders['train'],
                    elbo_fn,
