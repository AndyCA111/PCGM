...
dataloaders = setup_dataloaders(batch, f_id=0, Test=True)##dataset 1
pgm_path = f'../../checkpoints/checkpoint.pt'
...
df.to_csv('cf_pa_data_synthseg_2025.csv', index=False)

# Convert list of dictionaries to a DataFrame
df = pd.DataFrame(pa_np_list)

# Save DataFrame to CSV
df.to_csv('pa_data_synthseg_2025.csv', index=False)
