...
class NumpyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Return the data and the file's basename
...
class Unet3D(nn.Module):
    def __init__(
        ...
        self.init_conv0 = nn.Conv3d(channels, init_dim, (init_kernel_size, init_kernel_size, init_kernel_size),
...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, img_lr=None, cond=None, cond_scale=1., batch_size=16, DDIM=True):
        ...
        return self.p_sample_loop((batch_size, channels, num_frames, image_size[0], image_size[1]), cond=cond, img_lr=img_lr,
    ...
    def p_losses(self, x_start, x_start_lr, t, indexes=None, cond=None, noise=None, **kwargs):
        ...
        if self.loss_type == 'l1':
            ...
            # print(x_start.shape, x_recon.shape)
...
class Trainer(object):
    def __init__(
        ...
        self.ds = NumpyDataset(eval_folder)
    ...
    def save(self, milestone):
        self.accelerator.save_state(str(self.results_folder / f'final_ckpt'))

    def load(self, milestone, **kwargs):
        if milestone == -1:
            dirs = os.listdir(self.results_folder)
            dirs = [d for d in dirs if d.endswith("final_ckpt")]
    ...
    def train(
        ...
        self.results_folder = os.path.join(str(self.results_folder), "uncond_diffdecoder_tmi")
        ...
        for i, batch in tqdm(enumerate(self.dl)):
            with torch.no_grad():
                    ...
                    img_lr = F.interpolate(img, size=(157, 192, 160), mode='trilinear', align_corners=True)
                    ...
                    condition_emb = condition_emb.to(self.accelerator.device)
                    batches = num_to_groups(num_samples, self.batch_size)
                    basename = batch[1][0].replace('.npy', '.nii')
                    path_nii = os.path.join(str(self.results_folder), basename)
                    print(path_nii)
                    if os.path.exists(path_nii):
                        print(f"{path_nii} already exists. Skipping...")
                        continue
                    all_videos_list = list(map(lambda n: self.ema_model.sample(batch_size=n, img_lr=img_lr, cond=condition_emb), batches))
                    # all_videos_list = list(map(lambda n: self.ema_model.module.sample(batch_size=n, img_lr=img_lr, cond=condition_emb), batches))
                    all_videos_list = torch.cat(all_videos_list, dim=0)
                    save_nii(all_videos_list[0].unsqueeze(dim=0), path_nii)
            if i >= 400:  # Adjust the number as needed
                print("Stopping after 10 batches")
                break
...
trainer = Trainer(diffusion_model=diffusion_model,
                  ...
                  results_folder='./high_resolu',
                  eval_folder = '/scratch/m000065/binxu/gcp/mnt/disks/VLM/samples/opensora/results_sdvae_192_age_sex/uncond_sample_mia_diffdecoder_tmi',
                  num_sample_rows=1,
                  max_grad_norm=1.0)
