...
from dataloader_ad import get_data_loaders, get_metadata
...
def extract_age_from_metadata(metadata_df, basic_brain_path, output_filename):
    basic_fname = os.path.basename(basic_brain_path)  # e.g., LAB_S00205-20080514.nii.gz
    output_fname = os.path.basename(basic_brain_path)  # recover original filename if needed
    basic_age_row = metadata_df[metadata_df['filename'].str.contains(basic_fname)]
    new_age_row = metadata_df[metadata_df['filename'].str.contains(output_fname)]
    if basic_age_row.empty or new_age_row.empty:
        raise ValueError(f"Cannot find age info for {basic_fname} or {output_fname} in metadata.")

    basic_brain_age = basic_age_row['age'].values[0]
    new_brain_age = new_age_row['age'].values[0]
    
    return basic_brain_age, new_brain_age
...
class Trainer(object):
    ...
    def sample_od(
        ...
        self.results_folder = os.path.join(str(self.results_folder), "hiv_sample")
        if not os.path.exists(self.results_folder) and self.accelerator.is_main_process:
            os.mkdir(self.results_folder)
        self.ema_model.eval()
        # self.ema_model.half()
        df = pd.read_csv('/home/wepeng/binx/brain_gen/Open-Sora-Plan/examples/subjects_cvpr_rebuttal.csv')
        for i, batch in tqdm(df.iterrows()):
            ...
            img_path = '/home/wepeng/data/MRI_3Set/Lab_data/LAB_S00394-20120828.nii.gz'
            # find max age brain:
            subject_id = None
            ...
            # ======================== get basic brain : ========================
            basic_brain_path = '/home/wepeng/data/MRI_3Set/Lab_data/LAB_S00043-20100928.nii.gz'
            output_filename = 'LAB_S00043-20100928-cf-2018.nii.gz'
            basic_brain_path = '/home/wepeng/data/MRI_3Set/Lab_data/LAB_S00205-20080514.nii.gz'
            output_filename = 'LAB_S00205-20080514-cf-2018.nii.gz'
            basic_brain_age = 40.841
            new_brain_age = 50.953
            ...
            new_age = (new_brain_age - age_mean) / age_std
            ...
            with torch.no_grad():
                ...
                # # #new start point
                inv_latents = list(
                    map(lambda n: self.ema_model.module.ddim_inv_sample(batch_size=n, latents=img, new_cond=condition_emb, cond_scale=0.5), batches))
                ...
                new_latents = list(
                    map(lambda n: self.ema_model.module.sample(batch_size=n, cond=new_condition_emb, cond_scale=1.5, startpoint=inv_latents[-(start_step + 1)][None]), batches))
    ...
    def sample_bc(
        ...
        # self.results_folder = os.path.join(str(self.results_folder), "hiv_sample")
        self.results_folder = '/home/wepeng/binx/brain_gen/Open-Sora-Plan/hiv_sample_2'
        if not os.path.exists(self.results_folder) and self.accelerator.is_main_process:
            os.makedirs(self.results_folder, exist_ok=True)
        # 添加 reconstruction 结果保存路径

        self.ema_model.eval()

        # === Load subject-case table and metadata ===
        filtered_df = pd.read_csv('/home/wepeng/binx/filtered_H_with_files.csv')
        metadata_lab, metadata_ncan, metadata_ad = get_metadata(path="/home/wepeng/data/MRI_3Set/", only_control=False)
        age_mean, age_std = 48.64647, 27.514878
        search_folder = '/home/wepeng/data/MRI_3Set/Lab_data'

        for _, row in tqdm(filtered_df.iterrows(), total=len(filtered_df)):
            subject_id = row['Subject']
            all_files = row['all_data'].split(',')

            age_map = {}
            sex_str = None
            for fname in all_files:
                print(fname)
                match = metadata_lab[metadata_lab['fname'].str.contains(fname)]
                if not match.empty:
                    age_map[fname] = match['age'].values[0]
                    print(age_map)
                    if sex_str is None:
                        sex_str = match['sex'].values[0]
                else:
                    print(f"[Warning] Age not found for {fname}, skipping.")
            if len(age_map) < 2 or sex_str not in {'M', 'F'}:
                continue
            all_img2_folder = '/home/wepeng/binx/brain_gen/Open-Sora-Plan/hiv_sample_real'
            if not os.path.exists(all_img2_folder) and self.accelerator.is_main_process:
                os.makedirs(all_img2_folder, exist_ok=True)
            for fname in all_files:
                if fname not in age_map:
                    continue
                try:
                    path = os.path.join(search_folder, fname)
                    data = nib.load(path).get_fdata()
                    data = np.clip(data, np.percentile(data, 5), np.percentile(data, 95))
                    data = (data / np.percentile(data, 95)) * 2 - 1
                    img2 = np.ones((160, 192, 157)) * (-1)
                    img2[11:11+138, 8:8+176, 9:9+138] = data

                    recon_output_filename = 'real-' + fname.replace('.nii.gz', '.nii.gz')
                    recon_output_path = os.path.join(all_img2_folder, recon_output_filename)
                    ni_img2 = nib.Nifti1Image(img2, affine=np.eye(4))
                    nib.save(ni_img2, recon_output_path)
                    print(f"[Saved img2] {recon_output_path}")
                except Exception as e:
                    print(f"[Error] {fname}: {e}")
            sex = 0 if sex_str == 'M' else 1

            baseline_fname = min(age_map, key=age_map.get)
            baseline_age = age_map[baseline_fname]
            baseline_path = os.path.join(search_folder, baseline_fname)

            data = nib.load(baseline_path).get_fdata()
            data = np.clip(data, np.percentile(data, 5), np.percentile(data, 95))
            data = (data / np.percentile(data, 95)) * 2 - 1
            img2 = np.ones((160, 192, 157)) * (-1)
            img2[11:11+138, 8:8+176, 9:9+138] = data
            img = np.transpose(img2, (2, 1, 0))
            img = torch.from_numpy(img[None]).float().repeat(3, 1, 1, 1).unsqueeze(0).to(self.accelerator.device, dtype=self.vae.dtype)
            z = self.vae.encode(img).sample() * 0.19

            age = (baseline_age - age_mean) / age_std
            cond = torch.cat([
                age * torch.ones(1, 250, 768),
                sex * torch.ones(1, 250, 768)
            ], dim=1).to(self.accelerator.device, dtype=torch.float32)

            inv_scales = [0.5, 1.0, 1.5]
            resample_scales = [0.75, 1.0, 1.5]
            best_score = float('inf')
            best_pair = None

            for inv_scale in inv_scales:
                inv_latents = self.ema_model.module.ddim_inv_sample(
                    batch_size=1, latents=z, new_cond=cond, cond_scale=inv_scale
                )
                for resample_scale in resample_scales:
                    rec_latents = self.ema_model.module.sample(
                        batch_size=1,
                        cond=cond,
                        cond_scale=resample_scale,
                        startpoint=inv_latents[-(start_step + 1)][None]
                    )
                    rec_latents = (rec_latents / 0.19).to(self.vae.dtype)
                    rec_img = self.vae.decode(rec_latents)
                    score = ((rec_img - img) ** 2).mean().item()

                    if score < best_score:
                        best_score = score
                        best_pair = (inv_latents, resample_scale)

            inv_latents, best_resample_scale = best_pair
            print(f"[Baseline] subject={subject_id} | baseline={baseline_fname} | best_mse={best_score:.4f}")

            for fname in all_files:
                if fname == baseline_fname or fname not in age_map:
                    continue
                output_filename = fname.replace('.nii.gz', '-cf.nii.gz')
                print(f'Editing file: {output_filename}, based on {baseline_fname}')

                new_age = age_map[fname]
                new_age_norm = (new_age - age_mean) / age_std
                new_cond = torch.cat([
                    new_age_norm * torch.ones(1, 250, 768),
                    sex * torch.ones(1, 250, 768)
                ], dim=1).to(self.accelerator.device, dtype=torch.float32)

                edited_latents = self.ema_model.module.sample(
                    batch_size=1,
                    cond=new_cond,
                    cond_scale=best_resample_scale,
                    startpoint=inv_latents[-(start_step + 1)][None]
                )
                output_path = os.path.join(self.results_folder, output_filename)
                save_nii(edited_latents[0].unsqueeze(0), self.vae, output_path)
    ...
    def sample(
        ...
        for _, row in tqdm(filtered_df.iterrows(), total=len(filtered_df)):
            ...
            for fname in all_files:
                ...
                target_path = os.path.join(search_folder, fname)
                try:
                    target_data = nib.load(target_path).get_fdata()
                    target_data = np.clip(target_data, np.percentile(target_data, 5), np.percentile(target_data, 95))
                    target_data = (target_data / np.percentile(target_data, 95)) * 2 - 1
                    target_img2 = np.ones((160, 192, 157)) * (-1)
                    target_img2[11:11+138, 8:8+176, 9:9+138] = target_data
                    target_tensor = np.transpose(target_img2, (2, 1, 0))
                    target_tensor = torch.from_numpy(target_tensor[None]).float().repeat(3, 1, 1, 1).unsqueeze(0).to(self.accelerator.device, dtype=self.vae.dtype)

                    # Save target real image
                    recon_output_filename = 'real-' + fname.replace('.nii.gz', '.nii.gz')
                    recon_output_path = os.path.join(all_img2_folder, recon_output_filename)
                    ni_img2 = nib.Nifti1Image(target_img2, affine=np.eye(4))
                    nib.save(ni_img2, recon_output_path)
                    print(f"[Saved img2] {recon_output_path}")

                except Exception as e:
                    print(f"[Error loading target] {fname}: {e}")
                    continue

                new_age = age_map[fname]
                old_age_norm = (baseline_age - age_mean) / age_std
                if new_age - baseline_age > 4:
                    print(new_age - baseline_age)
                    new_age_norm = (new_age - age_mean) / age_std
                else:
                    print(new_age - baseline_age)
                    new_age_norm = (new_age - age_mean) / age_std
                ...
                old_cond = torch.cat([
                        old_age_norm * torch.ones(1, 250, 768),
                        sex * torch.ones(1, 250, 768)
                    ], dim=1).to(self.accelerator.device, dtype=torch.float32)
                best_score = float('inf')
                best_resample_scale = None
                best_output_latents = None

                for inv_scale in inv_scales:
                    inv_latents = self.ema_model.module.ddim_inv_sample(
                        batch_size=1, latents=z, new_cond=old_cond, cond_scale=inv_scale
                    )
                    for resample_scale in resample_scales:
                        edited_latents = self.ema_model.module.sample(
                            batch_size=1,
                        ...
                        edited_latents_scaled = (edited_latents / 0.19).to(self.vae.dtype)
                        edited_img = self.vae.decode(edited_latents_scaled)

                        # Center crop for score calculation
                        crop_s = [28, 46, 28]  # Starting indices for 100x100x100 crop
                        crop_e = [128, 146, 128]  # Ending indices
                        edited_crop = edited_img[:, :, crop_s[0]:crop_e[0], crop_s[1]:crop_e[1], crop_s[2]:crop_e[2]]
                        target_crop = target_tensor[:, :, crop_s[0]:crop_e[0], crop_s[1]:crop_e[1], crop_s[2]:crop_e[2]]
                        score = ((edited_crop - target_crop) ** 2).mean().item()

                        if score < best_score:
                            best_score = score
                            best_resample_scale = resample_scale
                            best_output_latents = edited_latents

                output_filename = fname.replace('.nii.gz', '-cf.nii.gz')
                output_path = os.path.join(self.results_folder, output_filename)
                save_nii(best_output_latents[0].unsqueeze(0), self.vae, output_path)
                print(f"[Saved] {output_filename} | best_mse={best_score:.4f}")
                
    def sample_one_case(
        self,
        baseline_path='/home/wepeng/data/MRI_3Set/Lab_data/LAB_S00394-20120228.nii.gz',
        target_path='/home/wepeng/data/MRI_3Set/Lab_data/LAB_S00394-20180329.nii.gz',
        baseline_age=61.119,
        target_age=67.203,
        sex_str='F',
        output_name="sample_result",
        save_folder="/home/wepeng/binx/brain_gen/Open-Sora-Plan/hiv_sample_one",
        start_step=0,
    ):
        assert sex_str in {'M', 'F'}
        sex = 0 if sex_str == 'M' else 1
        age_mean, age_std = 48.64647, 27.514878
        inv_scales = [0.5, 1.0, 1.5, 2]
        resample_scales = [0.5, 1.0, 1.5, 2]
        ...
        def load_img_tensor(path):
            ...
            img2[11:11+138, 8:8+176, 9:9+138] = data
            img = np.transpose(img2, (2, 1, 0))
            img = torch.from_numpy(img[None]).float().repeat(3, 1, 1, 1).unsqueeze(0)
            return img.to(self.accelerator.device, dtype=self.vae.dtype), img2

        # === Load baseline image and encode ===
        img, _ = load_img_tensor(baseline_path)
        z = self.vae.encode(img).sample() * 0.19
        with torch.no_grad():
            recon_latents = z / 0.19
            recon_img = self.vae.decode(recon_latents)

        recon_img_np = recon_img[0, 0].cpu().numpy()
        recon_img_np = np.transpose(recon_img_np, (2, 1, 0))  # back to (H, W, D)
        recon_nii = nib.Nifti1Image(recon_img_np, affine=np.eye(4))
        recon_path = os.path.join(save_folder, output_name + "-recon.nii.gz")
        nib.save(recon_nii, recon_path)
        print(f"[Saved] {recon_path}")
        # === Load target image ===
        target_tensor, target_img2 = load_img_tensor(target_path)
        target_tensor = target_tensor.to(self.accelerator.device, dtype=self.vae.dtype)

        # === Save target image ===
        ni_img2 = nib.Nifti1Image(target_img2, affine=np.eye(4))
        target_save_path = os.path.join(save_folder, output_name + "-real.nii.gz")
        nib.save(ni_img2, target_save_path)

        # === Build conditioning ===
        old_age_norm = (baseline_age - age_mean) / age_std
        new_age_norm = (target_age - age_mean) / age_std
        new_cond = torch.cat([
            new_age_norm * torch.ones(1, 250, 768),
            sex * torch.ones(1, 250, 768)
        ...
        # === Save best result ===
        output_path = os.path.join(save_folder, output_name + "-cf.nii.gz")
        save_nii(best_output_latents[0].unsqueeze(0), self.vae, output_path)
        print(f"[Saved] {output_path} | best_mse={best_score:.4f}")
...
trainer = Trainer(diffusion_model=diffusion_model,
                  ...
                  results_folder='/mnt/disks/VLM/samples/opensora/results_sdvae_192_age_sex',
                  num_sample_rows=1,
                  max_grad_norm=1.0)

trainer.load(-1)
trainer.sample_one_case()