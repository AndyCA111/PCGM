...
class Trainer(object):
    ...
    def sample(
        ...
        self.results_folder = os.path.join(str(self.results_folder), "test_age_cf_sample_3")
        ...
        for i, batch in tqdm(df.iterrows()):
            ...
            if fname.startswith("NCANDA"):
                ...
                parts = fname.split('_')
                if len(parts) > 2:
                    subject_id = f"{parts[1]}_{parts[2].split('-')[0]}"
            ...
            new_age = (new_age - age_mean) / age_std
            ...
            with torch.no_grad():
                ...
                new_latents = list(
                    map(lambda n: self.ema_model.module.sample(batch_size=n, cond= new_condition_emb, cond_scale=4, startpoint=inv_latents[-(start_step + 1)][None]), batches))
