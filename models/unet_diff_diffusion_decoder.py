        ...
        while self.step < self.train_num_steps:
            for i in range(self.gradient_accumulate_every):
                with self.accelerator.accumulate(self.model):
                    ...
                    f = img.shape[2]
                    img_latents = self.vae.encode(img).sample()                
                    if sex == 'M':
                        sex = 0
                    else:
                        sex = 1
                    ...
                    img_lr = F.interpolate(img_latents, size=(157, 192, 160), mode='trilinear', align_corners=True)
                    img = img[:,0].unsqueeze(dim=1)
                    text = condition_emb

                    B, C, D, H, W = img_lr.shape
