...
def Upsample(dim):
    return nn.ConvTranspose3d(dim, dim, (4, 4, 4), (1, 2, 2), (0, 1, 1))

def Downsample(dim):
    return nn.Conv3d(dim, dim, (4, 4, 4), (1, 2, 2), (0, 1, 1))
...
class Trainer(object):
    ...
    def save(self, milestone):
        self.accelerator.save_state(str(self.results_folder / f'final_ckpt_2'))
        ...
        while self.step < self.train_num_steps:
            for i in range(self.gradient_accumulate_every):
                with self.accelerator.accumulate(self.model):
                    ...
                    batch = next(self.dl)
                    img_batch, seg_batch, seg_mask_name, age, sex, _, _, _, _ = batch
                    if sex == ['M']:
                        sex = -1
                    elif sex == ['F']:
                        sex = 1
                ...
                if self.accelerator.sync_gradients:
                    ...
                    with torch.no_grad():
                        if self.step != 0 and self.step % (self.save_and_sample_every) == 0:
                            ...
                            #     map(lambda n: self.ema_model.sample(batch_size=n, cond=age_emb, cond_scale=2), batches))
                            all_videos_list = list(
                                map(lambda n: self.ema_model.module.sample(batch_size=n, cond=condition_emb, cond_scale=1.2), batches))
                            ...
                            path_nii = str(self.results_folder / str(f'{str(milestone)}_{file_name}_age_{age}_sex_{sex}.nii'))
...
trainer = Trainer(diffusion_model=diffusion_model,
                #   folder="/ocean/projects/asc170022p/yanwuxu/diffusion/data/R3/img_segs_128",
                  ...
                  results_folder='./results_sdvae_192_age_sex_AD',
                  num_sample_rows=1,
                  max_grad_norm=1.0)
