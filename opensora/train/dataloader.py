...
def get_metadata(path="/home/wpeng/data/MRI_High/"):
    seg_path = '/scratch/project_2001654/Wpeng/JIAQI/SynthSeg/data/'
    ...
    df_sri['seg_fname'] = 'Lab_seg/'+ df_sri['subject']+'-'+df_sri['visit'].str.split('_').str[0]+'_synthseg.nii.gz'
    ...
    for index, row in df_ncanda.iterrows():
        if int(row['visit'])==0:
            ...
            seg_fn = 'NCANDA/'+ row['subject']+ "_baseline_synthseg.nii.gz"
            # print(fn)
        else:
            fn = 'NCANDA/'+ row['subject']+ f"_followup_{row['visit']}y.nii.gz"
            seg_fn = 'NCANDA/'+ row['subject']+ f"_followup_{row['visit']}y_synthseg.nii.gz"
        path_here = path + fn
        seg_path_here = seg_path + seg_fn
        if not os.path.exists(path_here) or not os.path.exists(seg_path_here):
            rows_to_remove.append(index)
   ...
   # Corrected expression, add file names
    ...
    df_ncanda['seg_fname'] = ''
    # Assign 'baseline.nii.gz' to rows where 'visit' is 0, string 0::::  Not working: df_ncanda['visit'] == '0'
    df_ncanda.loc[df_ncanda['visit'].astype(str)=='0', 'seg_fname'] = 'NCANDA/' + df_ncanda['subject'] +'_baseline_synthseg.nii.gz'
    # Assign follow-up filename to rows where 'visit' is not 0
    df_ncanda.loc[df_ncanda['visit'].astype(int)!=0, 'seg_fname'] = 'NCANDA/' + df_ncanda['subject'] + "_followup_"+df_ncanda['visit'].astype(str)+"y_synthseg.nii.gz"
    ...
    for index, row in df_adni.iterrows():
        ...
        seg_fn = 'ADNI_extract/'+ row['PTID'] +'-'+ row['EXAMDATE']+ '-' + 't1_synthseg.nii.gz'
        path_here = path + fn
        print(path_here)
        seg_path_here = seg_path + seg_fn
        if not os.path.exists(path_here) or not os.path.exists(seg_path_here):
    ...
    df_adni['seg_fname'] = 'ADNI_extract/'+ df_adni['PTID']+'-'+ df_adni['EXAMDATE'] + '-' +'t1_synthseg.nii.gz'
    # print(df_adni['fname'])
    ### Output only the Age sex, diagnosis
    df_adni['diagnosis']= df_adni['DX_bl']
    df_adni['site']= 2

    df_ADNI = df_adni[['fname', 'age', 'sex', 'diagnosis', 'site', 'seg_fname']]
    print(df_ADNI)
...
video_data_paths_dict = {
    ...
    "synthseg" : "/scratch/project_2001654/Wpeng/JIAQI/SynthSeg/data/",
...
def create_dataset(dataset_name, img_size=176, T=None, prompt='a mri brain image', deterministic=False, num_workers=1, return_dataset=False, ifexample = False,resize = False, age_normalize=False, crop_size=None, crop=True):
    data_path = video_data_paths_dict[dataset_name]
    T = default_T_dict[dataset_name] if T is None else T
    if dataset_name == "MRI":
        dataset = MRIDataset(train=True, imgsize=img_size, path=data_path, shard=0, num_shards=0, T=T, prompt=prompt, ifexample = ifexample,resize=resize, age_normalize= age_normalize,crop_size = crop_size,crop=crop)
    else:
...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        path, age, seg_path = self.getitem_path_age(idx)
        self.cache_file(path)
        try:
            # video = self.loaditem(path)
            seg = self.loaditem_seg(seg_path)
...
class MRIDataset(BaseDataset):
    def __init__(self, path, train=None, shard=None, num_shards=None, T=None, resize=False, imgsize =176, prompt ='a brain image',ifexample=False,age_normalize=False, crop_size=None, crop=True):
    ...
    def loaditem(self, path, random_transpose=True):
        ############## preprocessing ############## 
        img = nib.load(path)
        data = img.get_fdata()
        filename = str(path)
        ...
        if self.imgsize == 176:
            ...
            img2 = np.ones((160, 192, 153))*(-1)
            img2[11:11+138,8:8+176,7:7+138] = data
            
        else :
            img2 = img2
            
        if random_transpose:
            axes = np.random.permutation(len(img2.shape))
        else:
            axes = (2, 1, 0) 
        img = np.transpose(img2, axes)
    ...
    def loaditem_seg(self, path):
        ...
        selected_name, selected_values = random.choice(list(selected_lists.items()))
    
        processed_data = func(img, selected_values)
        if self.crop:
            processed_data = random_crop(processed_data, self.crop_size)
    ...
    def getitem_path_age(self, idx):
        return self.path / self.fnames[idx], self.ages[idx], self.seg_path / self.seg_fnames[idx], 
...
def get_data_loaders(
    ...
    dataset = create_dataset(dataset_name, img_size=img_size, prompt=prompt, ifexample = ifexample,resize=resize,age_normalize=age_normalize, crop_size=crop_size, crop=crop)
