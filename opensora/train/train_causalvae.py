...
class TrainingArguments:
    ...
    video_num_frames: int = field(
        default=69, metadata={"help": "The number of frames per video."}
    )
    nii_frames: int = field(
        default=89, metadata={"help": "The number of frames per video."}
    )
    nii_hw: int = field(
        default=96, metadata={"help": "The number of frames per video."}
    )
    ...
    dataset: str = field(
        default='video', metadata={"help": "choose modality you want to train."}
    )
...
def train(args):
    ...
    if args.load_from_checkpoint is not None:
        model = CausalVAEModel.load_from_checkpoint(args.load_from_checkpoint, strict =False)
    ...
    dataset = get_data_loaders("MRI", 176, 1, ifreturn_loader=False, ifexample =True, resize=False, crop=True, crop_size=(101, 104, 104))
